...
def cut_mp3_file(input_mp3, output_mp3, start_time, end_time):
    ...
    print(f"Cutting {input_mp3.name}: {start_time}s to {end_time}s (duration: {duration}s)")
...
def main():
    ...
    for transcript_path in transcript_files:
        ...
        if not all([video_id, start_time is not None, end_time is not None]):
            print(f"✗ Skipping {transcript_path.name}: missing video_id, start_time, or end_time")
            failed_count += 1
            continue

        # Check if corresponding mp3 exists
        input_mp3 = mp3_dir / f"{video_id}.mp3"
        if not input_mp3.exists():
            print(f"✗ Skipping {transcript_path.name}: mp3 file not found at {input_mp3}")
            failed_count += 1
            continue

        # Output file name
        output_mp3 = output_dir / f"{video_id}_clip_{int(start_time)}_{int(end_time)}.mp3"
